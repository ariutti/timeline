...
int mostRecentTouch = -1;
int mostRecentRelease = -1;
...
void setup()
{
  ...
  //int initialPos = (((1.0 * 0)/NPADS) * (NLEDS-SIDELOBE) ) + SIDELOBE;
  //int initialPos = (((1.0 * 0.5)/NVIRTUALPADS) * (NLcapiscoEDS-SIDELOBE) ) + SIDELOBE;
  int initialPos = (((7 * 1.0)/NVIRTUALPADS) * (NLEDS) );
  vehicle.setTarget(initialPos);  
}
    ...
    if( mpr[i].currtouched != mpr[i].lasttouched ) 
    {
      ...
      for(int j=0; j<NPADS; j++) // cycle through all the electrodes
      {
        if (( mpr[i].currtouched & _BV(j)) && !(mpr[i].lasttouched & _BV(j)) )
        {
          ...
          //centroidLeds = (((1.0 * centroidPads)/NPADS) * (NLEDS-SIDELOBE) ) + SIDELOBE;
          ...
          if( interpolatedPad != -1) {
            centroidLeds = (((1.0 * interpolatedPad)/NVIRTUALPADS) * (NLEDS - 2*SIDELOBE +1))+SIDELOBE;
            
            vehicle.setTarget(centroidLeds);
            if( DEBUG ) {
              Serial.print("centroids: PAD ");
              Serial.print(interpolatedPad);
              Serial.print(", LEDs ");
              Serial.print(centroidLeds);
              Serial.println();
            }
          }
        }
      ...
      //Serial.println( interpolatedPad );
    }
