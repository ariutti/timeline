...
int NUMBER_OF_VIRTUAL_PADS = 0;
int realPadIndex, virtualPadIndex;
...
unsigned long WAIT_TO_SEND = 10;
unsigned long startWaiting;
boolean TOUCH_STABILIZED = false;
...
boolean B_TO_PROCESSING	= false;
boolean B_USE_LEDS			= true;
...
void setup()
{
  ...
  int NUMBER_OF_REAL_PADS = 0;
  for(int i=0; i<NMPR; i++) {
    NUMBER_OF_REAL_PADS += NPADS[i];
  }
  NUMBER_OF_VIRTUAL_PADS = (NUMBER_OF_REAL_PADS * 2) - 1;

  if(DEBUG) {
    Serial.print("NMPR: ");
    Serial.print( NMPR );
    Serial.print(", NPADS tot: ");
    Serial.print( NUMBER_OF_REAL_PADS );
    Serial.print(", NUMBER_OF_VIRTUAL_PADS: ");
    Serial.print(NUMBER_OF_VIRTUAL_PADS);
    Serial.println(";");
  }
  ...
  // LED STUFF *********************************************************************/
  //if(B_USE_LEDS) carriage.init(&strip, NUMBER_OF_VIRTUAL_PADS);
  if(B_USE_LEDS) carriage.init(&strip, 25);
}
...
void loop()
{
    ...
    if( mpr[i].currtouched != mpr[i].lasttouched )
    {
      ...
      for(int j=0; j<NPADS[i]; j++) // cycle through all the electrodes
      {
        if (( mpr[i].currtouched & _BV(j)) && !(mpr[i].lasttouched & _BV(j)) )
        {
          ...
          // get the index of the real pad
          realPadIndex= composeIndex(i, j);
          touchObj.addIndex( realPadIndex );
          virtualPadIndex = touchObj.getInterpolatedIndex();

          newIdx = virtualPadIndex;

          TOUCH_STABILIZED = false;
        ...
        else if (!(mpr[i].currtouched & _BV(j)) && (mpr[i].lasttouched & _BV(j)) )
        {
          ...
          touchObj.removeIndex( realPadIndex );
          virtualPadIndex = touchObj.getInterpolatedIndex();
  ...
  if(newIdx != -1) {
    ...
    if(millis() - startWaiting > WAIT_TO_SEND && !TOUCH_STABILIZED)
    {
      TOUCH_STABILIZED = true;
...
void sentToVVVV(int activeVirtualPad) {
  /*
  for( int i=0; i<NUMBER_OF_VIRTUAL_PADS; i++) {
